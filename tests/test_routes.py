...
class AuditRouteTests(AppTestCase):
    ...
    def test_audit_details_include_readable_labels_and_ignore_empty_changes(
        ...
        with session_scope(self.app) as database:
            ...
            self.assertEqual(event.details["client"], f"Sample Client (ID: {seed.client_id})")
            ...
            with session_scope(self.app) as database:
                ...
                record_audit_event(
                    ...
                    details={
                        ...
                        "contract": f"Sample Contract (ID: {seed.contract_id})",
                    },
                )
                self.assertEqual(
...
class ReportAndSessionRouteTests(AppTestCase):
    ...
    def test_live_client_report_is_permanent_and_admin_shared(self) -> None:
        ...
        self.assertEqual(
            ...
            ["Your live time and cost report password for Sample Client has been reset"],
        )
    ...
    def test_my_sessions_lists_pending_and_finalized_rows_with_pagination(self) -> None:
        ...
        self.assertIn(b"Pending Invoice Sessions by Day", page.data)
        self.assertIn(b'data-label="Cost" data-pending-day-cost', page.data)
    ...
    def test_my_sessions_daily_cost_splits_midnight_and_preserves_total(self) -> None:
        ...
        with session_scope(self.app) as database:
            ...
            task = database.get(Task, self.seed.other_task_id)
            assert admin is not None and task is not None
            task.contract.hourly_rate_cents = 10001
            database.add(
                TimeEntry(
                    user=admin,
                    task=task,
                    started_at=datetime(2026, 7, 10, 4, 30),
                    stopped_at=datetime(2026, 7, 10, 5, 30),
                )
            )
        page = self.client.get("/sessions")
        self.assertEqual(page.status_code, 200)
        for day, expected in (
            (b"2026-07-09", b"$50.01"),
            (b"2026-07-10", b"$50.00"),
        ):
            row = re.search(
                rb'data-pending-day="' + day + rb'".*?</tr>', page.data, re.S
            )
            assert row is not None
            self.assertIn(expected, row.group())
        self.assertIn(b"$100.01", page.data)
    ...
    def test_my_sessions_tracks_running_pending_time_and_empty_state(self) -> None:
        ...
        self.assertIn(b"data-snapshot-day=", running.data)
