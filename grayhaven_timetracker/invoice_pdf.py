...
def _daily_summary_rows(
    ...
    current_day = _local(invoice.range_start_utc, timezone).date()
    final_day = _local(
        invoice.range_end_utc - timedelta(microseconds=1), timezone
    ).date()
...
def render_invoice_pdf(
    ...
    *,
    ...
    for day, hours in _daily_summary_rows(invoice, lines, timezone):
        daily_data.append(
            [
    ...
    worker_totals: dict[int, tuple[str, int]] = {}
    for line in lines:
        worker_name, total_seconds = worker_totals.get(
            line.user_id, (line.worker_name, 0)
        )
        worker_totals[line.user_id] = (
            worker_name,
    ...
    for worker_name, total_seconds in worker_totals.values():
        worker_data.append(
            [
                Paragraph(_text(worker_name), body),
                Paragraph(
                    str(
                        (Decimal(total_seconds) / Decimal(3600)).quantize(
